...
def change_password(new_password, name_db):
    config_data[name_db]["password"] = new_password
                ...
                for key, value in config_data[section].items():
                    if not section == "database":
                        ...
                        if section == "postgres" and key == "password":
                            change_password(new_value, "database")
                        ...
                        if key == "password":
                            ...
                            if key == "log_file_path" and not os.path.exists(new_value):
                                while not os.path.exists(new_value):
                                    print(f"Файл '{new_value}' не найден.")
                                    new_value = input(
                                        f"Введите новое значение для '{key}' (для выхода введите 'exit'): ")
                                    if new_value.lower() == "exit":
                                        break
                            config_data[section][key] = new_value

                            if section == "database" and key == "password":
                                change_password(new_value, "postgres")
                else:
                    break
