...
def check_func():
    try:
        ...
        if not database_exists:
            ...
            print("Main: База данных Practice успешно создана.")
            conn.commit()
        else:
            print("Main: База данных Practice уже существует.")

        cursor.close()
        conn.close()

        if not check_db_connetion(db_config_main):
            print("Ошибка: вероятно не правильно указанный пароль для PostgreSQL")
            sys.exit()
        ...
        if not access_logs_table_exists:
            ...
            print("Main: Таблица Access_logs успешно создана.")
        else:
            print("Main: Таблица Access_logs уже существует.")
        ...
        if not add_access_log_function_exists:
            ...
            cursor.execute("""
                ...
                $$ LANGUAGE plpgsql;
            """)
            print("Main: Функция add_access_log успешно создана.")
        else:
            print("Main: Функция add_access_log уже существует.")

        conn.commit()
        cursor.close()
        conn.close()

        print('\nMain: [Postgres настроен и готов к работе!]')
